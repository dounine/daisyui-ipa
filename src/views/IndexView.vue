...
<template>
  <div class="flex flex-wrap">
    <div v-for="item in list"
         class="w-full lg:w-1/4 py-2 lg:p-4 transition-transform duration-300 hover:-translate-y-px hover:translate-x-px">
      <div class="flex flex-row bg-base-100 shadow rounded-md">
        <div class="flex flex-row flex-1 my-2 ml-2">
          ...
          <div class="flex-col flex-1 justify-around ml-2">
            <div class="flex items-center w-full lg:w-24">
              <strong class="font-bold truncate">{{ item.name }}</strong>
            </div>
            <div class="flex items-center text-sm size">
              <p class="text-accent">国区</p>
            </div>
            <div class="flex items-center space-x-2 text-accent text-sm size">
              <p>288MB</p>
              <p class="border-l border-base-300 h-3"></p>
              <p>1.2.2</p>
            </div>
          </div>
        </div>
        <div class="flex items-center">
          <i class="icon icon-download text-2xl cursor-pointer hover:text-info mr-2"></i>
        </div>
      </div>
    </div>
  </div>
...
.shadow-app {
  box-shadow: 1px 1px 10px 0 rgb(136 167 225 /10%), -1px -1px 10px 0 #dadada;
}
